...
c Jixie: changed SET_THINGS_UP() to take beam energy as argument
      call SET_THINGS_UP(Ebeam,TMOD)

cJixieL changed XCUT to XCUT=A*0.99      
      XCUT=0.99D0         ! for NH3 
      IF (TARG.EQ.'ND3') THEN
...
c12     format(F6.3,1x,F6.3,1x,F7.3,1x,11(E12.5,1x))
...
CJixie: turn off MSC for phi angle
...
ckpppp          write(61,41) p_el,(theta_el-thCor)*raddeg,theta_el*raddeg,
ckpppp     <         (phi_el-thCor/sin(THD*radcon))*raddeg,phi_el*raddeg
c     =================1/27/13 ===========================================================================
